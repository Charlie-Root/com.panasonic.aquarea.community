...
/**
 * Error raised by the client.
 *
 * `code` carries the HTTP status or the Aquarea application error code when
 * there is one. `authFailed` marks the failures that no retry can fix — wrong
 * credentials, 2FA, a login flow that never yields an authorization code, a
 * missing access token — so a caller can react to the KIND of failure instead
 * of grepping the message text (which is not a stable contract and changes
 * with every upstream wording tweak).
 */
class AquareaError extends Error {
  constructor(message, code, { authFailed = false } = {}) {
    super(message);
...
class AquareaClient {
  ...
  async _doAuthenticate() {
    ...
    if (!code) throw new AquareaError('login: authorization code not obtained', undefined, { authFailed: true });
  ...
  async _loginWithCredentials(authorizeLocation) {
    ...
    if (pageRes.status !== 200) {
      throw new AquareaError(
        `login page: expected 200, got ${pageRes.status}. The Panasonic auth flow may have changed. Body: ${await this._peek(pageRes)}`
      );
    }
    ...
    if (loginRes.status !== 200) {
      throw new AquareaError(
        `usernamepassword/login: expected 200, got ${loginRes.status} (invalid credentials or 2FA?): ${await this._peek(loginRes)}`,
        loginRes.status, { authFailed: true }
      );
    }
    const formParams = parseHiddenInputs(await loginRes.text());
    if (!formParams.wresult) {
      throw new AquareaError('login: callback form not found (wrong credentials, or 2FA enabled?)',
        undefined, { authFailed: true });
    }
  ...
  _panasonicHeaders(includeClientId = true) {
    if (!this.accessToken) throw new AquareaError('access token missing', undefined, { authFailed: true });
  ...
  async _request(method, path, { json, headers, throwOnError = true } = {}, _retried = false) {
    ...
    if (res.status === 429) {
      throw new AquareaError('Rate-limited (HTTP 429). Increase the polling interval.', 429);
    }
    if (res.status < 200 || res.status >= 300) {
      // A 401/403 we already retried with a freshly minted token is not a
      // transient blip: the account itself is refusing us.
      const authFailed = _retried && (res.status === 401 || res.status === 403);
    ...
    if (throwOnError && data) {
      ...
      // Same reasoning as the 401/403 path above: an app-level token error we
      // already retried with a fresh token is a persistent auth failure.
      if (errs.length) {
        throw new AquareaError(errs[0].message, errs[0].code,
          { authFailed: Boolean(authErr) && _retried });
      }
    }
    return data;
  }
  ...
  /**
   * Writes a status patch on the unit (operationStatus, forceDHW, forceHeater,
   * forcedefrost, specialStatus), through the accsmart "transfer" proxy.
   *
   * ⚠️  Two shapes exist for these commands and only one works here:
   *      - aquarea-smart WEB app: POST `/remote/v1/api/devices/{guid}` with
   *        `status: [{ deviceGuid, ... }]`. That API authenticates with its
   *        own session token. POSTed straight to accsmart it answers HTTP 403
   *        "Authentication Token missing"; wrapped in the transfer envelope it
   *        answers HTTP 400, because the proxy resolves the target device from
   *        `bodyParam.gwid` and there is none.
   *      - Comfort Cloud transfer proxy: apiName `/remote/v1/api/devices` with
   *        `bodyParam: { gwid, ... }`. This is the one every other write in
   *        this client uses, and the one setMode() already uses to send
   *        `operationStatus: 1` when powering the unit back on.
   *
   * @param {string} deviceId
   * @param {object} patch  Status fields to write (excluding gwid).
   */
  async _directStatus(deviceId, patch) {
    const data = await this._transfer(
      '/remote/v1/api/devices',
      'POST',
      Object.assign({ gwid: deviceId }, patch),
    );
    // The proxy answers HTTP 200 even when the inner call was refused, and a
    // refusal is not always shaped like a `message[]` error: log the raw body
    // so a silently ignored command can still be diagnosed.
    this.log(`AquareaClient: status write ${JSON.stringify(patch)} -> ${JSON.stringify(data)}`);
    return data;
  }
   ...
   * @param {string} deviceId
   * @param {number} temp
   * @param {object} [state] Device data from a recent getDeviceData(). Only
   *   `hasTank` and `zoneId` are read, both structural. Passing it saves a
   *   round-trip on an API this app is deliberately rate-limit-shy about; when
   *   omitted the state is fetched, so standalone callers keep working.
   */
  async setTargetTemperature(deviceId, temp, state) {
    const value = Math.round(Number(temp));
    if (Number.isNaN(value)) throw new AquareaError('setTargetTemperature: invalid temperature');

    const current = state || await this.getDeviceData(deviceId);
   ...
   * @param {object} [state] Device data from a recent getDeviceData(). Only
   *   `hasTank` and `zoneId` are read, both structural. Passing it saves a
   *   round-trip; when omitted the state is fetched.
   */
  async setMode(deviceId, mode, state) {
    if (mode === 'off') {
...
/**
 * True for an authentication failure that no retry can fix: the device needs
 * fresh credentials from the user. Mirrors AquareaHomeClient.isAuthFailure so
 * both drivers share one detection policy.
 */
module.exports.isAuthFailure = err => Boolean(err) && err.authFailed === true;
