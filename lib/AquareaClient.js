...
// 'heat' et 'heat_tank' partagent le meme mode climat (chauffage) ;
...
const HOMEY_TO_UPDATE_MODE = { heat: 2, heat_tank: 2, cool: 3, cool_tank: 3, auto: 8 };
...
class AquareaClient {
  ...
  async getDeviceData(deviceId) {
    ...
    if (tank && this._num(tank.operationStatus) === 1) {
      if (thermostatMode === 'heat') thermostatMode = 'heat_tank';
   ...
   * @param {string} deviceId
   * @param {'auto'|'heat'|'heat_tank'|'cool'|'cool_tank'|'off'} mode
  ...
  async setMode(deviceId, mode) {
    ...
    if (current.hasTank) {
      if (mode === 'heat_tank' || mode === 'cool_tank') bodyParam.tankStatus = { operationStatus: 1 };
