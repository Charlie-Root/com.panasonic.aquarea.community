...
class AquareaClient {
   ...
   * forcedefrost, specialStatus).
   *
   * ⚠️  These commands used to be POSTed straight to
   *     `accsmart.panasonic.com/remote/v1/api/devices/{guid}` with the
   *     `status: [{ deviceGuid, ... }]` body of the aquarea-smart WEB app.
   *     That is a different API: aquarea-smart authenticates with its own
   *     session token, not with the Comfort Cloud bearer, so accsmart answered
   *     HTTP 403 "Authentication Token missing" on every call (power on/off,
   *     forced DHW, backup heater, defrost, eco/comfort preset).
   *     Everything else in this client already goes through the accsmart
   *     "transfer" proxy with the `{ gwid, ... }` body, so these commands now
   *     do the same.
   *
  ...
  _directStatus(deviceId, patch) {
    return this._transfer('/remote/v1/api/devices', 'POST', Object.assign({ gwid: deviceId }, patch));
  }
